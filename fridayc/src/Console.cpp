...
namespace friday::inline core::inline IO {
  ...
  FILE* Console::S_stderr = stderr;
  FILE* Console::S_stddbg = stderr;
  FILE* Console::S_stdlog = stdout;
  FILE* Console::S_stdnte = stdout;
  FILE* Console::S_stdwrn = stderr;
  ...
  auto Console::set_channel(Channel channel, FILE* stream) noexcept -> void {
    switch(channel) {
      case Channel::LOG: {
