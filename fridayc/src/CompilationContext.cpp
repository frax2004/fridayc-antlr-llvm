...
namespace friday::inline api::inline pipeline {
  ...
  CompilationContext::CompilationContext(vector<string> const& paths) {
    this->M_globalNamespace = Namespace::create("");
    ...
    | views::transform(&future<TranslationUnit*>::get)
    | ranges::to<vector>();

    auto intType = make_shared<PrimitiveType>(*this->M_global, "int", llvm::Type::getInt64Ty(this->M_llvmContext));
    auto byteType = make_shared<PrimitiveType>(*this->M_global, "byte", llvm::Type::getInt8Ty(this->M_llvmContext));
    auto boolType = make_shared<PrimitiveType>(*this->M_global, "bool", llvm::Type::getInt1Ty(this->M_llvmContext));
    auto floatType = make_shared<PrimitiveType>(*this->M_global, "float", llvm::Type::getDoubleTy(this->M_llvmContext));
    auto voidType = make_shared<PrimitiveType>(*this->M_global, "void", llvm::Type::getVoidTy(this->M_llvmContext));
  ...
  auto CompilationContext::print() -> void {
    ...
    auto table = "[{}, {}]"_f.format(
      ...
      this->M_namespaces
      | views::values
      | views::transform(&Namespace*::operator*)
  ...
  auto CompilationContext::get_units() const -> ranges::ref_view<const vector<TranslationUnit*>> {
    return this->M_units;
  }

  auto CompilationContext::add_unit(TranslationUnit* unit) -> void {
    this->M_units.push_back(unit);
  }

  auto CompilationContext::get_or_emplace_namespace(string_view name) -> Namespace* {
  ...
  auto CompilationContext::get_global() const -> Namespace* {
    return this->M_global;
  }

  auto CompilationContext::find_namespace(string_view name) -> Namespace* {
    auto it = this->M_namespaces.find(name);
    return it != this->M_namespaces.end() ? it->second : Namespace*{};
  }
